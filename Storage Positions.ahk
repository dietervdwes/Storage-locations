;TrakCare Data entry Script / Retrieval script
;Written by Dieter van der Westhuizen 08-05-2020 - updated various times - last update 2023-06-26
...
Gui, add, button,x2 y66 w75 h20, Storage
Gui, Add, button, x2 y88 w75 h20, Epis_Machine
...
{   ;This part of code reads and parses the CSV / txt file and stores results of the read file in an array.
    LineNumber := A_Index
    ;memskip := 2 ;Set this number to specify every how many times it should exit Results Entry.
    ;memcycle := 1 ; Just a variable which will cycle.
    Loop, parse, A_LoopReadLine, CSV
        ...
        if (A_Index = 73 or A_Index = 146 or A_Index = 218 or A_Index = 290 or A_Index = 360 or A_Index = 430 or A_Index = 500 or A_Index = 570 or A_Index = 666 or A_Index = 740 or A_Index = 814 or A_Index = 888 or A_Index = 962 or A_Index = 1036 or A_Index = 1110 or A_Index = 1184 or A_Index = 1258 or A_Index = 1332 or A_Index = 1406 or A_Index = 1480 or A_Index = 1554)
            {
            sleep, 500
            WinClose, Result Entry
            WinWaitClose, Result Entry
            sleep, 500
            WinActivate, NHLS-LABTRAK
            WinWaitActive, NHLS-LABTRAK
            sleep, 500
            MouseClick, Left, 76, 148, 1
            sleep, 500
            Send, a{TAB}
            sleep, 500
            Send, Result Entry
        ...
        send, {Enter}
        sleep, 500
        If WinActive("Search Unsuccessful")
        {
            send, {Enter}
            sleep, 500
            send, {AltDown}l{AltUp}
            FileAppend, %Episode_out%`,%Result%`,%A_Now%`,Search Unsuccessful`n, covlog.txt
            continue
        }
        else
        MouseClick, Left, 150, 300
        ...
        sleep, 800
        FileAppend, %Episode_out%`,%Result%`,%A_Now%`n, covlog.txt
        ;sleep, 3000
        WinActivate, Result Entry
        ;WinWaitActive, Result Entry
        sleep, 500
...
MsgBox, Please confirm that Specimen Information Window is open and "Storage Data" has been ticked.
FileName := storage_samples.csv
Loop, read, storage_samples.csv, ;output_list.txt ; output_list.txt is the file to write to, if necessary
{   ;This part of code reads and parses the CSV / txt file and stores results of the read file in an array.
    LineNumber := A_Index
    ;memskip := 2 ;Set this number to specify every how many times it should exit Results Entry.
    ;memcycle := 1 ; Just a variable which will cycle.
    Loop, parse, A_LoopReadLine, CSV
        EpisodeLine := LineNumber
        FileReadLine, Line_out, storage_samples.csv, EpisodeLine
        LineArray := StrSplit(Line_out, ",")
        Episode_out := LineArray[1]
        Result := LineArray[2] 
        sleep, 100
        ...
        sleep, 200
        If !WinActive("Specimen Information")
        {
           MsgBox, Window Specimen Information is not active.  Please open it, enter an episode number and tick "Storage Data".
            continue
        }
        ;MouseClick, Left, 105, 112, 2, 100
        ;MouseClick, Left, 105, 500, 2, 100
        ...
        WinWaitActive, Episode Information
        sleep, 300
		...
		sleep, 300
        WinClose, Episode Information
        sleep, 300
        FileAppend, %Episode_out%`, %clipboard% `,`n, specimen_log.txt
        sleep, 300
}

ButtonEpis_Machine:

MsgBox, Please confirm that Specimen Information Window is open and "Instrument Send/Test result data" has been ticked.
FileName = "episode_numbers.csv"
Loop, read, episode_numbers.csv, ;output_list.txt ; output_list.txt is the file to write to, if necessary
{   ;This part of code reads and parses the CSV / txt file and stores results of the read file in an array.
    LineNumber := A_Index
    ;memskip := 2 ;Set this number to specify every how many times it should exit Results Entry.
    ;memcycle := 1 ; Just a variable which will cycle.
    Loop, parse, A_LoopReadLine, CSV
        EpisodeLine := LineNumber
        FileReadLine, Line_out, episode_numbers.csv, EpisodeLine
        LineArray := StrSplit(Line_out, ",")
        Episode_out := LineArray[1]
        Result := LineArray[2] 
        sleep, 100
        ...
        If !WinActive("Specimen Information")
        {
           MsgBox, Window Specimen Information is not active.  Please open it, enter an episode number and tick "Instrument Send/Test result data".
            continue
        }
        ;MouseClick, Left, 105, 112, 2, 100
        ;MouseClick, Left, 105, 500, 2, 100
        ...
        WinClose, Episode Information
        sleep, 100
        FileAppend, %Episode_out%`, %clipboard% `n, %A_WorkingDir%\machine_data\%Episode_out%.txt
        sleep, 100
}
